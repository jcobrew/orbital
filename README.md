...
- **Dashboard query params** (compose with AND):
  `?canonicalType=<type>&q=<text>&type=<label>&country=<country>&status=<status>&focus=<text>&sort=<field>`.
  Example: `/dashboard?canonicalType=accelerator&country=USA&status=open`. (Filtering is by
  **Program Type** / `canonicalType` — the old `dataset` toggle is retired.)
